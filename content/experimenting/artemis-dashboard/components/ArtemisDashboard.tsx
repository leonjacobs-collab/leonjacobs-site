...
type CentreView  = 'orion' | 'gyro' | 'map';

interface KpEntry  { time_tag: string; kp_index: number; }
interface MagEntry { time_tag: string; bt: number; bz_gsm: number; proton_density?: number; speed?: number; }
interface FlareEntry { beginTime: string; classType: string; sourceLocation?: string; }
interface CmeEntry   { startTime: string; cmeAnalyses?: Array<{ speed: number }>; }
interface PolledResult<T> { data: T; live: boolean; }
interface Cell2D { ch: string; color: string; }
...
const CENTRE_VIEWS: CentreView[] = ['orion', 'gyro', 'map'];
const VIEW_META: Record<CentreView, { code: string; label: string }> = {
  orion: { code: 'ATT', label: 'ATTITUDE · ORION MPCV' },
  gyro:  { code: 'GYR', label: 'GYROSCOPE · ARTIFICIAL HORIZON' },
  map:   { code: 'MAP', label: 'NAV MAP · EARTH–MOON TRAJECTORY' },
};
...
function StatusDot({ status }: { status: Status }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: '50%', background: c, boxShadow: `0 0 5px ${c}`, flexShrink: 0,
    }} />
  );
}
...
function Cell({
  ...
  return (
    <div style={{
      ...
      fontFamily: FONT, ...outerStyle,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4, flexShrink: 0 }}>
        <span style={{
          background: P.amber, color: P.carbon,
          fontWeight: 700, fontSize: '0.58em', padding: '1px 3px', letterSpacing: '0.12em',
        }}>{code}</span>
        <span style={{ color: P.enamel, fontSize: '0.65em', letterSpacing: '0.07em', flex: 1, opacity: 0.7 }}>
          {title.toUpperCase()}
        </span>
        <StatusDot status={status} />
      </div>
      <div style={{ flex: 1, overflow: 'hidden', fontSize: '0.7em' }}>{children}</div>
    </div>
  );
}
...
function EventLog({ events }: { events: string[] }) {
  ...
  if (!events.length) return <div style={{ color: P.muted, fontSize: '0.9em' }}>NO EVENTS</div>;
  ...
  return (
    <div style={{ fontSize: '0.9em' }}>
      {visible.map((e, i) => (
        <div key={i} style={{
          color: i === 0 ? P.enamel : P.muted, marginBottom: 2,
...
// ─── Centre view renderers ────────────────────────────────────────────────────

/** Render a 2D char grid as JSX rows */
function Grid2D({ grid }: { grid: Cell2D[][] }) {
  return (
    <pre style={{ margin: 0, padding: 0, lineHeight: 1.35, fontSize: '0.8em', fontFamily: FONT, userSelect: 'none' }}>
      {grid.map((row, ri) => (
        <div key={ri}>
          {row.map((cell, ci) => (
            <span key={ci} style={{ color: cell.color }}>{cell.ch}</span>
          ))}
        </div>
      ))}
    </pre>
...
/** Build the gyroscope / artificial horizon grid */
function buildGyroGrid(rollDeg: number, pitchDeg: number): Cell2D[][] {
  const W = 38, H = 13;
  const rollRad    = clamp(rollDeg, -80, 80) * (Math.PI / 180);
  const pitchShift = pitchDeg * 0.22; // rows per degree of pitch
  ...
  for (let r = 0; r < H; r++) {
    ...
    for (let c = 0; c < W; c++) {
      const x = c - W / 2;
      const y = (r - H / 2) + pitchShift;

      // Horizon at this column (signed: positive = ground side)
      const horizY  = Math.tan(rollRad) * x;
      const dist    = y - horizY;

      // Fixed centre reticle: ─ ⊕ ─
      const centreRow = r === Math.floor(H / 2);
      const centreCol = Math.abs(c - W / 2);

      let ch: string, color: string;

      if (centreRow && centreCol === 0) {
        ch = '⊕'; color = P.amber;
      } else if (centreRow && centreCol <= 5) {
        ch = '─'; color = P.amber;
      } else if (Math.abs(dist) < 0.6 && centreCol > 5) {
        // Horizon line
        ch = '─'; color = P.cyan;
      } else if (dist > 0) {
        // Ground
        const depth = clamp(dist / 4, 0, 1);
        ch    = depth > 0.55 ? '▓' : '░';
        color = `hsl(30, 60%, ${8 + depth * 8}%)`;
      } else {
        // Sky
        const depth = clamp(-dist / 5, 0, 1);
        ch    = depth < 0.25 ? '·' : ' ';
        color = depth < 0.25 ? '#1a3a5a' : P.bg;
      }
...
/** Build the Earth–Moon nav-map grid */
function buildMapGrid(transProg: number, earthDist: number, moonDist: number): Cell2D[][] {
  const W = 38, H = 13;
  const MID   = Math.floor(H / 2);
  const ECOL  = 3;
  const MCOL  = W - 4;
  const AMP   = 3;   // rows of arc amplitude
  const PATHW = MCOL - ECOL - 1;

  const grid: Cell2D[][] = Array.from({ length: H }, () =>
    Array.from({ length: W }, () => ({ ch: ' ', color: 'transparent' })),
  );

  const set = (r: number, c: number, ch: string, color: string) => {
    if (r >= 0 && r < H && c >= 0 && c < W) grid[r][c] = { ch, color };
  };

  // Earth body
  set(MID - 1, ECOL, '╭─╮', P.cyan);   // can't use multi-char, set individually
  set(MID - 1, ECOL,     '╭', P.cyan);
  set(MID - 1, ECOL + 1, '─', P.cyan);
  set(MID - 1, ECOL + 2, '╮', P.cyan);
  set(MID,     ECOL,     '│', P.cyan);
  set(MID,     ECOL + 1, '⊕', P.cyan);
  set(MID,     ECOL + 2, '│', P.cyan);
  set(MID + 1, ECOL,     '╰', P.cyan);
  set(MID + 1, ECOL + 1, '─', P.cyan);
  set(MID + 1, ECOL + 2, '╯', P.cyan);

  // Moon body
  set(MID - 1, MCOL - 1, '╭', P.muted);
  set(MID - 1, MCOL,     '─', P.muted);
  set(MID - 1, MCOL + 1, '╮', P.muted);
  set(MID,     MCOL - 1, '│', P.muted);
  set(MID,     MCOL,     '●', P.muted);
  set(MID,     MCOL + 1, '│', P.muted);
  set(MID + 1, MCOL - 1, '╰', P.muted);
  set(MID + 1, MCOL,     '─', P.muted);
  set(MID + 1, MCOL + 1, '╯', P.muted);

  // Outbound arc (Earth → Moon, curves above midline)
  let scRow = MID, scCol = ECOL;
  for (let i = 0; i <= PATHW; i++) {
    const s   = i / PATHW;
    const col = ECOL + 3 + i;    // start clear of Earth box
    if (col >= MCOL - 2) break;  // stop clear of Moon box
    const row = Math.round(MID - Math.sin(s * Math.PI) * AMP);
    const traversed = s <= transProg;
    const isSpacecraft = Math.abs(s - transProg) < (1.8 / PATHW);
    if (isSpacecraft) {
      scRow = row; scCol = col;
      set(row, col, '◆', P.amber);
    } else {
      set(row, col, traversed ? '─' : '·', traversed ? P.amber : P.faint);
    }
  }

  // Return arc (Moon → Earth, curves below midline) — future path, faint
  for (let i = 0; i <= PATHW; i++) {
    const s   = i / PATHW;
    const col = MCOL - 3 - i;
    if (col <= ECOL + 2) break;
  ...
  // Labels row — above Earth, above Moon
  set(0, ECOL,     'E', P.cyan);
  set(0, ECOL + 1, 'A', P.cyan);
  set(0, ECOL + 2, 'R', P.cyan);

  set(0, MCOL - 1, 'M', P.muted);
  set(0, MCOL,     'O', P.muted);
  set(0, MCOL + 1, 'N', P.muted);

  // Distance annotation near spacecraft
  const distStr = `${Math.round(earthDist / 1000)}k`;
  const annotCol = Math.min(scCol + 1, W - distStr.length - 1);
  const annotRow = scRow <= MID ? scRow + 1 : scRow - 1;
  for (let i = 0; i < distStr.length; i++) {
    set(annotRow, annotCol + i, distStr[i], P.amber);
  }
...
async function fetchFlares(): Promise<FlareEntry[]> {
  const end = new Date(), start = new Date(end.getTime() - 7 * 86_400_000);
  const fmt = (d: Date) => d.toISOString().slice(0, 10);
  const r = await fetch(`https://kauai.ccmc.gsfc.nasa.gov/DONKI/WS/get/FLR?startDate=${fmt(start)}&endDate=${fmt(end)}`);
...
async function fetchCme(): Promise<CmeEntry[]> {
  const end = new Date(), start = new Date(end.getTime() - 7 * 86_400_000);
  const fmt = (d: Date) => d.toISOString().slice(0, 10);
  const r = await fetch(`https://kauai.ccmc.gsfc.nasa.gov/DONKI/WS/get/CME?startDate=${fmt(start)}&endDate=${fmt(end)}`);
...
export default function ArtemisDashboard() {
  ...
  const t   = 3 * 86_400 + met;   // offset to day-3 trans-lunar coast

  // ── Centre panel view cycling ─────────────────────────────────────────────
  const [centreView, setCentreView] = useState<CentreView>('orion');

  useEffect(() => {
    const id = setInterval(() => {
      setCentreView(v => {
        const idx = CENTRE_VIEWS.indexOf(v);
        return CENTRE_VIEWS[(idx + 1) % CENTRE_VIEWS.length];
      });
    }, 8_000);
    return () => clearInterval(id);
  }, []);
  ...
  const advanceView = () =>
    setCentreView(v => CENTRE_VIEWS[(CENTRE_VIEWS.indexOf(v) + 1) % CENTRE_VIEWS.length]);
  ...
  // ── Solar wind ────────────────────────────────────────────────────────────
  const lastMag   = magRes.data.length ? magRes.data[magRes.data.length - 1] : { bt: 4.2, bz_gsm: -0.8, speed: 415 };
  ...
  const flareEvents = [...flareRes.data].slice(-10).reverse().map(f => {
    const d = f.beginTime.slice(5, 16).replace('T', ' ');
    return `${d} ${f.classType}${f.sourceLocation ? ' ' + f.sourceLocation : ''}`;
  });
  const cmeEvents = [...cmeRes.data].slice(-8).reverse().map(c => {
  ...
  const utcStr = `${padN(dn.getUTCHours())}:${padN(dn.getUTCMinutes())}:${padN(dn.getUTCSeconds())} UTC`;
  const metStr = formatMET(t);

  // ── Centre view grids (memoised per frame for gyro/map) ───────────────────
  const gyroGrid = centreView === 'gyro' ? buildGyroGrid(roll, pitch) : null;
  const mapGrid  = centreView === 'map'  ? buildMapGrid(transProg, earthDist, moonDist) : null;

  // ── Layout helper ─────────────────────────────────────────────────────────
  const grid = (col: string, row: string): React.CSSProperties => ({ gridColumn: col, gridRow: row });

  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <div style={{
      position: 'fixed', inset: 0,
      background: P.bg, color: P.enamel, fontFamily: FONT, fontSize: 11,
      display: 'flex', flexDirection: 'column', overflow: 'hidden', zIndex: 0,
    }}>

      {/* ── Header ── */}
      <div style={{
        background: P.headerBg, borderBottom: `2px solid ${P.amber}`,
        padding: '8px 14px', paddingTop: 72,   // 72px clears the fixed site nav
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{
            background: P.amber, color: P.carbon, fontWeight: 700,
            fontSize: '0.8em', padding: '2px 6px', letterSpacing: '0.15em',
          }}>ART</span>
          <div>
            <div style={{ color: P.enamel, fontSize: '0.85em', letterSpacing: '0.15em', fontWeight: 700 }}>ARTEMIS II</div>
            <div style={{ color: P.muted,  fontSize: '0.62em', letterSpacing: '0.1em' }}>MISSION INSTRUMENT PANEL</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, fontSize: '0.75em' }}>
          <div><span style={{ color: P.muted }}>UTC </span><span style={{ color: P.cyan }}>{utcStr}</span></div>
          <div><span style={{ color: P.muted }}>MET </span><span style={{ color: P.amber }}>{metStr}</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <span style={{
              display: 'inline-block', width: 7, height: 7, borderRadius: '50%',
              background: anyLive ? P.green : P.yellow, boxShadow: `0 0 6px ${anyLive ? P.green : P.yellow}`,
            }} />
      ...
      <div style={{
        ...
        {/* ── Centre console (spans cols 2–3, rows 1–3) ── */}
        <div style={{
          ...grid('2 / 4', '1 / 4'),
          background: P.panel, border: `1px solid ${P.borderHi}`,
          display: 'flex', flexDirection: 'column', padding: '6px 10px', overflow: 'hidden',
        }}>
          {/* Header row: badge + title + view dots */}
          <div style={{
            ...
            <span style={{
              background: P.amber, color: P.carbon,
              fontWeight: 700, fontSize: '0.58em', padding: '1px 3px', letterSpacing: '0.12em',
            }}>{VIEW_META[centreView].code}</span>
            <span style={{ color: P.enamel, fontSize: '0.65em', letterSpacing: '0.07em', opacity: 0.7, flex: 1 }}>
              {VIEW_META[centreView].label}
            </span>
            {/* View selector — click to advance */}
            <button
              onClick={advanceView}
              title="Cycle view"
              style={{
                background: 'none', border: 'none', cursor: 'pointer',
                display: 'flex', gap: 4, alignItems: 'center', padding: 0,
              }}
            >
              {CENTRE_VIEWS.map(v => (
                <span key={v} style={{
                  display: 'inline-block', width: 6, height: 6,
                  borderRadius: '50%',
                  background: v === centreView ? P.amber : P.faint,
                  boxShadow:  v === centreView ? `0 0 5px ${P.amber}` : 'none',
                  transition: 'all 0.3s',
                }} />
              ))}
            </button>
          ...
          {/* RPY readouts — always visible */}
          <div style={{ display: 'flex', gap: 20, flexShrink: 0, marginBottom: 5, fontSize: '0.78em' }}>
            {[{ label: 'ROLL', val: roll }, { label: 'PITCH', val: pitch }, { label: 'YAW', val: yaw }].map(({ label, val }) => (
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {centreView === 'orion' && (
              <pre style={{
                ...
                fontSize: '0.78em', fontFamily: FONT, userSelect: 'none',
              }}>
            ...
            {/* GYR — artificial horizon */}
            {centreView === 'gyro' && gyroGrid && (
              <div style={{ width: '100%' }}>
                <div style={{ color: P.muted, fontSize: '0.6em', letterSpacing: '0.1em', marginBottom: 4, textAlign: 'center' }}>
                  ROLL {roll >= 0 ? '+' : ''}{roll.toFixed(1)}°&nbsp;&nbsp;
                  PITCH {pitch >= 0 ? '+' : ''}{pitch.toFixed(1)}°&nbsp;&nbsp;
                  YAW {yaw >= 0 ? '+' : ''}{yaw.toFixed(1)}°
                </div>
                <Grid2D grid={gyroGrid} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6em', color: P.muted, marginTop: 3 }}>
                  <span style={{ color: P.cyan }}>─── HORIZON</span>
                  <span style={{ color: P.amber }}>⊕ RETICLE</span>
                  <span style={{ color: '#4a2a00' }}>▓ GROUND</span>
                </div>
              </div>
            )}

            {/* MAP — Earth–Moon trajectory */}
            {centreView === 'map' && mapGrid && (
              <div style={{ width: '100%' }}>
                <Grid2D grid={mapGrid} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6em', marginTop: 4 }}>
                  <span style={{ color: P.cyan }}>⊕ EARTH  {Math.round(earthDist).toLocaleString()} km</span>
                  <span style={{ color: P.amber }}>◆ ORION  {(transProg * 100).toFixed(1)}%</span>
                  <span style={{ color: P.muted }}>● MOON  {Math.round(moonDist).toLocaleString()} km</span>
                </div>
              </div>
            )}
          </div>
        ...
        <Cell code="KP" title="Kp Index" status={kpStatus} style={grid('1', '2')}>
          ...
          <div style={{ marginTop: 5 }}><AsciiSpark data={kpHistory} width={18} /></div>
        </Cell>
        ...
        <Cell code="SWD" title="Solar Wind" status={bzStatus} style={grid('4', '2')}>
          ...
          <div style={{ marginTop: 4 }}><AsciiSpark data={bzHistory} width={18} /></div>
          {lastMag.bz_gsm < -5 && (
            <div style={{ color: P.red, fontSize: '0.8em', marginTop: 3, letterSpacing: '0.05em' }}>▲ GEOEFFECTIVE</div>
          )}
        </Cell>
        ...
        {/* FLR */}
        <Cell code="FLR" title="Solar Flares" status={flareRes.live ? 'nominal' : 'warning'} style={grid('1', '4')}>
          {flareEvents.length === 0
            ...
            : <div style={{ fontSize: '0.88em' }}>
                {flareEvents.slice(0, 4).map((e, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      color: i === 0 ? P.enamel : P.muted, marginBottom: 2, lineHeight: 1.3,
                      whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                    }}>
                      <span style={{ color: flareColor(cls) }}>{cls}</span>{e.replace(cls, '')}
                    </div>
                  );
                })}
              </div>
        ...
        <Cell code="ENV" title="Crew Env" style={grid('3', '4')}>
          ...
          <Readout label="CO₂"  value={co2Pct.toFixed(3)}    unit="%" color={co2Pct > 0.5 ? P.yellow : P.green} />
          ...
          {[
            ...
            <div key={name} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3, fontSize: '0.9em' }}>
              ...
              <span style={{
                ...
                background: P.green, boxShadow: `0 0 4px ${P.green}`, alignSelf: 'center',
              }} />
            </div>
          ))}
        </Cell>
